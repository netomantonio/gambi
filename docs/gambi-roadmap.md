...
- **Teto de ~120s para resposta NÃO-streaming (confirmado 2026-06-17, via wide event):** o gateway do StackSpot derruba a conexão (`RemoteProtocolError: Server disconnected`) em ~120s quando a resposta é bufferizada (`streaming:False`). Turnos agênticos longos estouravam isso → 502. **Fix (v1) ✅ VALIDADO no corp env (2026-06-17):** agent mode usa `streaming:True` + acumulação (`BufferedAgentStreamInvoker`); agent/plan mode funcionam fim-a-fim (tools, criar arquivos, planos). Mais lento que LLM nativo, mas completo. **Plano estrutural (sem teto + UX de raciocínio):** execução async via Quick Commands — ver §9 (item L do backlog).
